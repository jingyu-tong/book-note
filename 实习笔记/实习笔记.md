...
auto [lower, upper] =
  mmp.equal_range("four");
```
可以直接用 auto，捕获 pair、tuple 等的元素值，还是很方便的，注意vector 这种编译期长度位置的，不可以捕获。
...
为了实现服务注册以及服务发现的功能，Kite之间可以互相通信，通过Kontrol的服务发现机制，一个Kite可以发现其它的Kites。也就是说一个Kite可以在Kontrol注册自己，从而让其它的kites能找到它； Kontrol本身也是一个Kite，它用于对服务进行注册和鉴权，使用 etcd 作为存储。服务熔断，也可以在这里实现
...
当时我们考虑了两种方案：
* 两表连接查询
    这个方案有几个问题，一个是公司不鼓励使用 join，主要是由于join 的优化可能有性能上的问题，且可能会同时锁住多个表
* 读取两次，在 service 层做处理
