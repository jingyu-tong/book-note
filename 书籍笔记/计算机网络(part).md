...
- [计算机网络](#计算机网络)
	...
	- [网络层(数据层面)](#网络层数据层面)
		- [1.IPv4数据报](#1ipv4数据报)
		- [2.IPv4编址](#2ipv4编址)
		- [3.网络地址转换(NAT)](#3网络地址转换nat)
	- [网络层(控制层面)](#网络层控制层面)
		- [1.内部网管协议(RIP)](#1内部网管协议rip)
		- [2.开放最短路径优先(OSPF)](#2开放最短路径优先ospf)
		- [3.边界网关协议(BGP)](#3边界网关协议bgp)
		- [4.ICMP因特网控制报文协议](#4icmp因特网控制报文协议)
	- [链路层](#链路层)
		- [1.地址解析协议(ARP)](#1地址解析协议arp)
...
在 http2 之前，http 的请求都是串行的，即使开启了长连接以及管线化，因为请求是顺序到达的，server 依然得一个个请求按顺序进行恢复，管线化也只是让发送请求不阻塞了而已。

但是这有可能问题，因为先发送的请求，如果处理的比较慢的话，还是会阻塞服务端处理后面的请求。

在一个 TCP 连接上，我们可以向对方不断发送帧，每帧的 stream identifier 的标明这一帧属于哪个流，然后在对方接收时，根据 stream identifier 拼接每个流的所有帧组成一整块数据。
把 HTTP/1.1 每个请求都当作一个流，那么多个请求变成多个流，请求响应数据分成多个帧，不同流中的帧交错地发送给对方，这就是 HTTP/2 中的多路复用。

为了标明是属于哪个留，Stream Identifier 为流 id，客户端建立的为奇数，服务端为偶数，来避免冲突。
...
### 4.拥塞控制
首先，当网络中的发送方发送了超出网络负载的数据时，即使路由器能够储存一些数据，但是速度持续高于负载时，路由器只能丢弃掉一些数据。

为了解决这个问题，TCP 采取了一定的策略来控制拥塞。

发送方限制未确认的数据不大于rwnd和cwnd的最小值，即:`LastByteSent - LastByteAcked <= min{cwnd, rwnd}`。

因为网络状况是不停的改变，所以和 rwnd 不同，cwnd 是需要根据时间而更新的。
...
TCP 为了实现超时重传，需要对 RTT(round time trip, 往返时间)进行估计，TCP 会对一些包进行采样，然后对估计的 RTT 进行滤波，得到 eRTT。此外，RTT 的波动范围也会被采用，并同样做滤波得到 devRTT。最后的超时时间间隔为 eRTT + 4devRTT。此外，一旦发生超时，超时间隔将会加倍，知道收到确认，或者重传超时。
